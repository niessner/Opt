...
CUDASolverSHLighting::CUDASolverSHLighting(int width, int height)
{
	...
	const unsigned int numoflecacheAlbedo = (width+LE_THREAD_SIZE-1)/LE_THREAD_SIZE*height*54;
	cutilSafeCall(cudaMalloc(&d_albedoestcashe, sizeof(float)*3*numoflecacheAlbedo));
	h_albedoestmat = new float[54];
}
...
CUDASolverSHLighting::~CUDASolverSHLighting()
{
	...
	cutilSafeCall(cudaFree(d_albedoestcashe));	
	delete [] h_albedoestmat;
}
...
{	
	...
	if(input.d_litprior != NULL)
	{
        ...
        float weight_prior = 10.0f; //GlobalAppState::get().s_weightPriorLight;
