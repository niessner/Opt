...
__inline__ __device__ float evalFDevice(unsigned int variableIdx, SolverInput& input, SolverState& state, SolverParameters& parameters)
{
	...
	if (validN0){ float3 q = state.d_x[get1DIdx(n0_i, n0_j, input.width, input.height)]; float3 v = (p - q); e_reg += make_float3(v.x*v.x, v.y*v.y, v.z*v.z); }
	if (validN1){ float3 q = state.d_x[get1DIdx(n1_i, n1_j, input.width, input.height)]; float3 v = (p - q); e_reg += make_float3(v.x*v.x, v.y*v.y, v.z*v.z); }
	if (validN2){ float3 q = state.d_x[get1DIdx(n2_i, n2_j, input.width, input.height)]; float3 v = (p - q); e_reg += make_float3(v.x*v.x, v.y*v.y, v.z*v.z); }
	if (validN3){ float3 q = state.d_x[get1DIdx(n3_i, n3_j, input.width, input.height)]; float3 v = (p - q); e_reg += make_float3(v.x*v.x, v.y*v.y, v.z*v.z); }
...
__inline__ __device__ float3 evalMinusJTFDevice(unsigned int variableIdx, SolverInput& input, SolverState& state, SolverParameters& parameters)
{
	...
	float3 p = state.d_x[get1DIdx(i, j, input.width, input.height)];
	float3 e_reg = make_float3(0.0f, 0.0f, 0.0f);
	if (validN0){ float3 q = state.d_x[get1DIdx(n0_i, n0_j, input.width, input.height)]; e_reg += 2.0f*(p - q); pre += 4.0f*parameters.weightRegularizer*make_float3(1.0f, 1.0f, 1.0f); }
	if (validN1){ float3 q = state.d_x[get1DIdx(n1_i, n1_j, input.width, input.height)]; e_reg += 2.0f*(p - q); pre += 4.0f*parameters.weightRegularizer*make_float3(1.0f, 1.0f, 1.0f); }
	if (validN2){ float3 q = state.d_x[get1DIdx(n2_i, n2_j, input.width, input.height)]; e_reg += 2.0f*(p - q); pre += 4.0f*parameters.weightRegularizer*make_float3(1.0f, 1.0f, 1.0f); }
	if (validN3){ float3 q = state.d_x[get1DIdx(n3_i, n3_j, input.width, input.height)]; e_reg += 2.0f*(p - q); pre += 4.0f*parameters.weightRegularizer*make_float3(1.0f, 1.0f, 1.0f); }
