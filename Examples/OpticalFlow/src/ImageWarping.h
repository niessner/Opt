...
public:
	ImageWarping(const ColorImageR32& sourceImage, const ColorImageR32& targetImage) {
		...
		const float sigmas[numLevels] = { 1.0f, 2.0f, 3.0f, 4.0f, 5.0f };
		...
		for (unsigned int i = 0; i < numLevels; i++) {
			...
			//LodePNG::save(ImageHelper::convertGrayToColor(targetFiltered), "filtered" + std::to_string(i) + ".png");
			m_levels[i].init(sourceImage, targetFiltered);
		}		
		resetGPU();
