...
CERESWarpingSolver::CERESWarpingSolver(unsigned int width, unsigned int height, unsigned int depth)
{
	...
	d_unknown = new vec3d[width * height * depth];
...
void CERESWarpingSolver::solve(
	float3* d_x, float3* d_a, float3* d_urshape, float3* d_constraints, unsigned int nNonLinearIterations, unsigned int nLinearIterations, unsigned int nBlockIterations, float weightFit, float weightReg)
{
	...
	auto getCell = [=](int x, int y, int z) {
		return z * m_width * m_height + y * m_width + x;
	};

	/*const int voxelCount = m_width * m_height * m_depth;
