...
PS_INPUT ComputeQuadVertex(uint x, uint y)
{
	...
	float4 posClip = mul(float4(posWorldCC.x, posWorldCC.y, 0.0f, posWorldCC.z), g_mIntrinsicNew);
	posClip = float4(posClip.x/posClip.w, posClip.y/posClip.w, posClip.w, 1.0f);
