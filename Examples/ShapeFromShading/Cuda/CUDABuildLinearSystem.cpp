...
CUDABuildLinearSystem::CUDABuildLinearSystem(unsigned int imageWidth, unsigned int imageHeight) 
{
	...
	unsigned int dimX = (unsigned int)ceil(((float)imageWidth*imageHeight)/(localWindowSize*blockSize));
	
	m_size = 30*sizeof(float)*dimX; // buffer is reused for all levels -> maximum of dimX*dimY==400 elements on each level
	cutilSafeCall(cudaMalloc(&d_output, m_size));
	h_output = new float[m_size];
}
...
void CUDABuildLinearSystem::applyBL(float4* input, float4* target, float4* targetNormals, float3& mean, float meanStDev, Eigen::Matrix4f& deltaTransform, unsigned int imageWidth, unsigned int imageHeight, unsigned int level, Matrix6x7f& res, LinearSystemConfidence& conf) 
{
	...
	buildLinearSystem(imageWidth, imageHeight, d_output, input, target, targetNormals, deltaTransform.data());
	cutilSafeCall(cudaMemcpy(h_output, d_output, m_size, cudaMemcpyDeviceToHost));
