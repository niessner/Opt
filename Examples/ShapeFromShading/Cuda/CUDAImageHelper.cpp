...
void CUDAImageHelper::applyProjectiveCorrespondences(
	...
	const Eigen::Matrix4f& deltaTransform, unsigned int imageWidth, unsigned int imageHeight, float distThres, float normalThres, float levelFactor, const mat4f& intrinsic) 
{
	projectiveCorrespondences(
		...
		(const float4x4*)&deltaTransform, (const float4x4*)&intrinsic);
}
