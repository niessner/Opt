...
HRESULT CUDARGBDAdapter::OnD3D11CreateDevice(ID3D11Device* device, RGBDSensor* RGBDSensor, unsigned int width, unsigned int height)
{
	...
	m_depthIntrinsics._m00 *= scaleWidthDepth;  m_depthIntrinsics._m03 *= scaleWidthDepth;
	m_depthIntrinsics._m11 *= scaleHeightDepth; m_depthIntrinsics._m13 *= scaleHeightDepth;
	...
	m_colorIntrinsics._m00 *= scaleWidthColor;  m_colorIntrinsics._m03 *= scaleWidthColor;
	m_colorIntrinsics._m11 *= scaleHeightColor; m_colorIntrinsics._m13 *= scaleHeightColor;
...
HRESULT CUDARGBDAdapter::process(ID3D11DeviceContext* context)
{
	...
	if (m_RGBDSensor->processDepth() == S_FALSE)	return S_FALSE; // Order is important!
	if (m_RGBDSensor->processColor() == S_FALSE)	return S_FALSE;
	...
	if((m_RGBDSensor->getColorWidth() == m_width) && (m_RGBDSensor->getColorHeight() == m_height)) copyFloat4Map(d_colorMapResampledFloat4, d_colorMapFloat4, m_width, m_height);
	else																						   resampleFloat4Map(d_colorMapResampledFloat4, m_width, m_height, d_colorMapFloat4, m_RGBDSensor->getColorWidth(), m_RGBDSensor->getColorHeight());
...
float* CUDARGBDAdapter::GetDepthMapResampledFloat()
{
	return d_depthMapResampledFloat;
}

float4* CUDARGBDAdapter::GetColorMapResampledFloat4()
{
	return d_colorMapResampledFloat4;
}

unsigned int CUDARGBDAdapter::getWidth() const
{
	return m_width;
}

unsigned int CUDARGBDAdapter::getHeight() const
{
	return m_height;
}
