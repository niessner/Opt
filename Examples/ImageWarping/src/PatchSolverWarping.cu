...
__global__ void PCGStepPatch_Kernel(PatchSolverInput input, PatchSolverState state, PatchSolverParameters parameters, int ox, int oy)
{
	...
	__shared__ float  PA[SHARED_MEM_SIZE_PATCH]; setPatchToZero(PA, tId_i, tId_j);
	...
	register float2 Delta  = make_float2(0.0f, 0.0f);
	register float  DeltaA = 0.0f;
	register float2 R;
	register float  RA;
	register float2 Z;
	register float  ZA;
	...
	if (isValidPixel)
	{
		R = evalMinusJTFDevice(tId_i, tId_j, gId_i, gId_j, W, H, constraintUV, M, U, X, XA, parameters, Pre, PreA, RA); // residuum = J^T x -F - A x delta_0  => J^T x -F, since A x x_0 == 0 
		float2 preRes  = Pre *R;																						// apply preconditioner M^-1
		float  preResA = PreA*RA;																					    // apply preconditioner M^-1 
		P [getLinearThreadIdCache(tId_i, tId_j)] = preRes;															    // save for later
		PA[getLinearThreadIdCache(tId_i, tId_j)] = preResA;																// save for later
	
		d = dot(R, preRes) + RA*preResA;
	}
	...
	{
		const float2 currentP  = P [getLinearThreadIdCache(tId_i, tId_j)];
		const float  currentPA = PA[getLinearThreadIdCache(tId_i, tId_j)];
	
		float d = 0.0f;
		if (isValidPixel)
		{
			AP = applyJTJDevice(tId_i, tId_j, gId_i, gId_j, W, H, constraintUV, M, U, P, PA, X, XA, parameters, APA);	// A x p_k  => J^T x J x p_k 
			d = dot(currentP, AP) + currentPA*APA;																		// x-th term of denominator of alpha
		}
		...
		if (isValidPixel)
		{
			...
			DeltaA = DeltaA + alpha*currentPA;								// do a decent step		
			R  = R  - alpha*AP;												// update residuum	
			RA = RA - alpha*APA;											// update residuum		
			Z  = Pre *R;													// apply preconditioner M^-1
			ZA = PreA*RA;													// apply preconditioner M^-1
		...
		if (isValidPixel)
		{
			...
			PA[getLinearThreadIdCache(tId_i, tId_j)] = ZA + beta*currentPA;						// update decent direction
		}
	...
	if (isValidPixel)
	{
		...
		state.d_A[get1DIdx(gId_i, gId_j, W, H)] = XA_CC + DeltaA;
	}
}
