...
public:
	ImageWarping(const ColorImageR32& image, const ColorImageR32& imageMask, std::vector<std::vector<int>>& constraints) : m_constraints(constraints){
		...
		m_warpingSolverTerraAD = new TerraSolverWarping(m_image.getWidth(), m_image.getHeight(), "ImageWarpingCombined.t", "gaussNewtonGPU");
	...
	ColorImageR32* solve() {
		...
		for (unsigned int i = 1; i < numIter; i++)	{
			std::cout << "//////////// ITERATION"  << i << "  (TERRA AD) ///////////////" << std::endl;
