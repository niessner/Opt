...
P:Image("X", opt.float,W,H,0)
P:Image("A", opt.float,W,H,1)
P:Stencil(2)
...
local terra cost(i : int32, j : int32, gi : int32, gj : int32, self : P:ParameterType()) : float
	...
	var l0 = laplacianCost(x,i+1,j+0,gi+1,gj+0,self.X)
	var l1 = laplacianCost(x,i+0,j+1,gi+0,gj+1,self.X)
	var l2 = laplacianCost(x,i-1,j+0,gi-1,gj+0,self.X)
	var l3 = laplacianCost(x,i+0,j-1,gi+0,gj-1,self.X)
	var laplacian = l0*l0 + l1*l1 + l2*l2 + l3*l3
		
	var e_reg = w_reg*laplacian
	
	var res : float = e_fit + e_reg
...
local terra gradient(i : int32, j : int32, gi : int32, gj : int32, self : P:ParameterType()) : unknownElement
	...
	var l0 = 2.0f*laplacianCost(x,i+1,j+0,gi+1,gj+0,self.X)
	var l1 = 2.0f*laplacianCost(x,i+0,j+1,gi+0,gj+1,self.X)
	var l2 = 2.0f*laplacianCost(x,i-1,j+0,gi-1,gj+0,self.X)
	var l3 = 2.0f*laplacianCost(x,i+0,j-1,gi+0,gj-1,self.X)	
...
local terra applyJTJ(i : int32, j : int32, gi : int32, gj : int32, self : P:ParameterType(), pImage : P:UnknownType()) : unknownElement
	...
	--reg
	var l0 = 2.0f*laplacianCost(p,i+1,j+0,gi+1,gj+0,pImage)
	var l1 = 2.0f*laplacianCost(p,i+0,j+1,gi+0,gj+1,pImage)
	var l2 = 2.0f*laplacianCost(p,i-1,j+0,gi-1,gj+0,pImage)
	var l3 = 2.0f*laplacianCost(p,i+0,j-1,gi+0,gj-1,pImage)	
	var laplacian = l0 + l1 + l2 + l3
	var e_reg = 2.0f*w_reg*laplacian
