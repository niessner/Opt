\begin{abstract}
Many graphics and vision problems are naturally phrased as optimizations with nonlinear least squares objective functions over visual data types like images.
...
We propose a new language in which a user simply writes energy functions over pixel and voxel-structured unknowns, and a compiler automatically generates state-of-the-art real-time optimization kernels, using any of multiple solver methods, on both CPUs and GPUs.
The end result is a system in which real-world energy functions in graphics and vision applications are expressible in tens of lines of code, but compile directly into highly optimized CPU and GPU solver implementations with performance competitive with the best published hand-tuned GPU solver implementations.
\end{abstract}
